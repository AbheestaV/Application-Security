...
def register_view(request):
    if request.method == 'GET':
        ...
        #we are saving information based on the password in plaintext. Do it with the hashed instead.
        #Added code in end of function to collect this information instead.
        #graphs[pword].inc()
        ...
        u.save()
        #Added code for collecting information here instead
        if hashed_pword not in graphs.keys():
            graphs[hashed_pword] = Counter(f'counter_{hashed_pword}', 'The total number of '\
              + f'times {hashed_pword} was used')
        graphs[hashed_pword].inc()
...
#No Password is being checked. Need to add a Password check as well.
def gift_card_view(request, prod_num=0):
    ...
    if request.method == "GET":
        ...
        if user is not None:
            context['failed'] = False
            login(request, user)
            print("Logged in user, can now gift")
        else:
            context['failed'] = True
            graphs['database_error_return_404'].inc()
