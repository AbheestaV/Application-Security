...
struct this_gift_card *gift_card_reader_buf(const uint8_t *buf, size_t DataSize)
    {
    ...
    // while (!feof(input_fd)) {
    for (iflag = 0; iflag < DataSize; iflag += 8)
    {
        struct gift_card_data *gcd_ptr;
        // JAC: Why aren't return types checked?
        // fread(&ret_val->num_bytes, 4,1, input_fd);
        // Added code
        if(sizeof(ret_val->num_bytes)>buf){
        memcpy(ret_val->num_bytes, buf, 4);
        }
        buf += 4;
        // End of added code
        ...
        if(sizeof(ret_val->num_bytes)>buf){
        memcpy(ptr, buf, ret_val->num_bytes);
        }
        buf += 4;
        optr = ptr - 4;
        //gcd_ptr = malloc(sizeof(struct this_gift_card));
